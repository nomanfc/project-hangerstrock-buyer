...
const Freight = () => {
  ...
  const handleOpenEditModal = (data) => (e) => {
    ...
    get_freights_by_id(data.row.original.id).then((res) => {
  ...
  const handleRemoveUser = () => {
    hide_freight(deleteData).then((res) => {
  ...
  const handleCreate = (e) => {
    if (addData.freight_name) {
      create_freight(addData).then((res) => {
        if (res.data.success === 1) {
          fetchData();
  ...
  const handleEdit = (e) => {
    if (editData.freight_name) {
      edit_freight(editData).then((res) => {
        if (res.data.success === 1) {
          fetchData();
      ...
      >
        <Box sx={ styleRemoveDel}>
          <Typography
            ...
            style={{ color: "#599f22" }}
          >
            Do you want to remove this freight type ?
          </Typography>
