...
const Condition = () => {
  ...
  const handleOpenEditModal = (data) => (e) => {
    ...
    get_conditions_by_id(data.row.original.id).then((res) => {
  ...
  const fetchData = useCallback(async () => {
    const res = await get_conditions();
  ...
  const handleRemoveUser = () => {
    hide_condition(deleteData).then((res) => {
  ...
  const handleCreate = (e) => {
    if (addData.condition_name) {
      create_condition(addData).then((res) => {
        if (res.data.success === 1) {
          fetchData();
  ...
  const handleEdit = (e) => {
    if (editData.condition_name) {
      edit_condition(editData).then((res) => {
        if (res.data.success === 1) {
          fetchData();
